...
public sealed class OllamaAIPromptExecutionSettings : PromptExecutionSettings
{
    /// <summary>
    /// Default: 0.7
    ...
    /// instance returned from <see cref="OllamaAIToolCallBehavior.RequiredFunctions"/>, called with
    ...
    /// set the property to <see cref="OllamaAIToolCallBehavior.EnableKernelFunctions"/> if the client should simply
    /// send the information about the functions and not handle the response in any special manner, or
    /// <see cref="OllamaAIToolCallBehavior.AutoInvokeKernelFunctions"/> if the client should attempt to automatically
    ...
    /// </remarks>
    public OllamaAIToolCallBehavior? ToolCallBehavior
    {
    ...
    {
        return new OllamaAIPromptExecutionSettings()
        {
    ...
    /// <returns>An instance of OllamaAIPromptExecutionSettings</returns>
    public static OllamaAIPromptExecutionSettings FromExecutionSettings(PromptExecutionSettings? executionSettings)
    {
        ...
        if (executionSettings is OllamaAIPromptExecutionSettings settings)
        {
            return settings;
        }
        ...
        var OllamaExecutionSettings = JsonSerializer.Deserialize<OllamaAIPromptExecutionSettings>(json, JsonOptionsCache.ReadPermissive);
    ...
    private OllamaAIToolCallBehavior? _toolCallBehavior;
